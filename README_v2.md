...
| func     | item                                        | ✓    |
| act      | store                                       | ✓    |
| type     | 类别: 1=激活码 2=徽章 3=对人道具 5=对贴道具 |      |
| page     |                                             |      |
| sub_type | 子类别，多个用逗号分隔                      |      |
...
- act：可选项有两个
  - `use`：使用自己仓库中的道具
  - `buy_and_use`：购买一个商店中的道具并使用
- id：根据 act 字段的取值：
  - 选`use`时：填写仓库中道具的id
  - 选`buy_and_use`时，填写商店中道具的id
- arg：根据使用的道具类型填写：
  - 使用的是"对人道具"时，填写目标用户id
  - 使用的是"对贴道具"时，将：`tid`,`pid`,`态度`，用`\t`连接；其中`态度`可选值为`-1`(反对),`0`(中立),`1`(支持)，目前除了选1时接着对该回复点赞会出现+2赞（但是刷新就没了）以外没发现该参数的其他影响。
